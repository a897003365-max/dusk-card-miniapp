...
Page({
  data: { error: '', tickets: 0, owned: 3, pitySSR: 0, pityUR: 0, rulesOpen: false, results: [], resultOpen: false, revealedCount: 0, activeReveal: null, revealPhase: 'idle', replayMode: false },
  ...
  onShow() {
    ...
    try {
      ...
      if (!variant || variant.count <= 0) throw new Error('还没有拥有这个形态，暂时不能重温');
      this.dismissReveal();
  ...
  onHide() { if (this.data.replayMode) this.closeResult(); else this.dismissReveal(); },
  onUnload() { if (this.data.replayMode) this.closeResult(); else this.dismissReveal(); },
  refresh() {
    const app = getApp();
  ...
  revealLetter(event) {
    ...
    this.setData({ activeReveal: { ...card, profile }, revealPhase: 'charge' });
    const phases = [
  ...
  showRevealPhase(phase) {
    ...
    if (phase === 'arrive' || phase === 'settled') {
      ...
      patch.activeReveal = { ...active, revealed: true };
    }
    this.setData(patch);
  },
  skipReveal() {
  ...
  closeResult() { this.dismissReveal(); this.setData({ resultOpen: false, replayMode: false }); },
